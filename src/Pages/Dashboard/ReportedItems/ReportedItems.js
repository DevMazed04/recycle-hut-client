...
const ReportedItems = () => {
   ...
   const {
      ...
      queryFn: async () => {
         const res = await fetch(
            "https://recycle-hut-server.vercel.app/reported-items"
         );
   ...
   const handleDeleteProduct = (reportedItem) => {
      fetch(
         `https://recycle-hut-server.vercel.app/reported-items/${reportedItem._id}`,
         {
            method: "DELETE",
         }
      )
         ...
         .then((data) => {
            if (data.deletedCount > 0) {
               refetch();
               toast.success(
                  `${reportedItem.productName} is deleted from Reported Item`
               );
            }
         });
   };
         ...
         {reportedItems?.length === 0 ? (
            ...
            <>
               <h2 className="text-3xl mb-5">
                  All Reported Items ({reportedItems?.length})
               </h2>
